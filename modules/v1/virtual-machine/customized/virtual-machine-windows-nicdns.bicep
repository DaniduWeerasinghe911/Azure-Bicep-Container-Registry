...
var configFilePath = 'https://stdckloudconf896674.blob.core.windows.net/dcconfig/configuration.zip'//'https://${dscStorageAccount}.blob.${environment().suffixes.storage}/${dscStorageAccountContainer}/configuration.zip'
...
resource extension_dscextension 'Microsoft.Compute/virtualMachines/extensions@2020-12-01' = [for i in range(0, vmCount): {
  name: '${vm[i].name}/dscextension'
  location: location
  properties: {
    ...
    settings: {
      ...
      configurationFunction: 'Configuration.ps1\\dckloudConfig'
      properties: {
     }
    }
  }
